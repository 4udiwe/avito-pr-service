...
type Request dto.PostPullRequestCreateJSONRequestBody
...
func (h *handler) Handle(ctx echo.Context, in Request) error {
	PR, err := h.s.CreatePR(ctx.Request().Context(), in.PullRequestId, in.PullRequestName, in.AuthorId)

	if err != nil {
		var errResponse dto.ErrorResponse

		if errors.Is(err, service.ErrAuthorNotFound) || errors.Is(err, service.ErrReviewerNotFound) {
		...
		if errors.Is(err, service.ErrPRAlreadyExists) {
			errResponse.Error.Code = dto.PREXISTS
			errResponse.Error.Message = "PR id already exists"
			return echo.NewHTTPError(http.StatusNotFound, errResponse)
		}
